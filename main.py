...
def get_coins(user_id):
    return DATA["coins"].get(str(user_id), INITIAL_COINS)
...
def normalize_variables(lua_code):
    obfuscated_patterns = [r'\b[a-zA-Z_]\w*_ref\d*\b', r'\b[a-zA-Z_]\w*_fn\b', r'\br\d+\b', r'\bn\d+\b', r'\bv\d+\b']
    ui_and_lua_blacklist = ["game", "workspace", "pairs", "unpack", "table", "wait", "env", "Color3", "string", "loadstring", "pcall", "true", "false", "items", "text", "flag", "state", "callback", "name", "value", "options", "default", "min", "max", "scroll", "visible", "enabled"]
    found_vars = []
    ...
    for i, line in enumerate(lines):
        ...
        if alias_match:
            ...
            valid_services = ["Players", "ReplicatedStorage", "RunService", "UserInputService", "Lighting", "Workspace", "CoreGui", "Teams", "SoundService", "StarterGui", "TweenService", "HttpService", "TeleportService"]
...
def beautify_lua(content):
    ...
    try:
        response = requests.post("https://relua.lua.cz/deobfuscate", json={"filename": "script.lua", "source": content, "lua_version": "Lua51", "pretty": PRETTY_MODE}, timeout=12)
...
async def on_message(message):
    ...
    if DATA["last_claim"].get(uid) != today:
        ...
        set_coins(uid, current + 50)
        DATA["last_claim"][uid] = today
        save_data(DATA)
        try:
            await message.author.send(f"Received 50 coins. Total: {get_coins(uid)}")
        except: pass
...
async def add_coin(interaction: discord.Interaction, member: discord.Member, amount: int):
    ...
    await interaction.response.send_message(f"Added {amount} to {member.name}.", ephemeral=True)

@bot.command(name="buycoin")
async def buy_coin(ctx):
    embed = discord.Embed(title="Purchase", description="Click to complete task.", color=discord.Color.green())
    view = discord.ui.View()
    view.add_item(discord.ui.Button(label="Task", url=TASK_URL, style=discord.ButtonStyle.link))
    await ctx.send(embed=embed, view=view)

@bot.command(name="coin")
async def check_coin(ctx):
    if ctx.author.id == FREE_USER_ID:
        await ctx.send("Infinite")
    else:
        await ctx.send(f"Balance: {get_coins(ctx.author.id)}")

@bot.command(name="topcoin")
async def top_coin(ctx):
    sorted_coins = sorted([(uid, amt) for uid, amt in DATA["coins"].items() if int(uid) != FREE_USER_ID], key=lambda x: x[1], reverse=True)[:10]
    if not sorted_coins:
        await ctx.send("Empty.")
        return
    embed = discord.Embed(title="Top 10", color=discord.Color.gold())
    desc = ""
    for i, (uid, amt) in enumerate(sorted_coins, 1):
        try:
            user = await bot.fetch_user(int(uid))
            desc += f"{i}. {user.name}: {amt}\n"
        except: continue
...
async def deobfuscate_cmd(ctx, *, args: str = None):
    ...
    if ctx.author.id != FREE_USER_ID and balance < COST:
        await ctx.message.reply(f"Insufficient funds. Need {COST}.")
        return
    content = None
    if ctx.message.attachments:
        ...
        stripped = args.strip("`")
        if stripped.startswith("http"): content = fetch_url(stripped)
        else: content = re.sub(r'^```[a-zA-Z]*\n|```$', '', stripped, flags=re.MULTILINE)
    if not content or not content.strip():
        await ctx.message.reply("No content.")
        return
    status = await ctx.message.reply("Processing...")
    output = beautify_lua(content)
    if ctx.author.id != FREE_USER_ID: set_coins(ctx.author.id, balance - COST)
    final = f"-- Created by 8xmj https://discord.gg/swjkGWeDM --\n\n{output}"
    await status.delete()
    await ctx.message.reply(content=f"{ctx.author.mention} Done.", file=discord.File(io.BytesIO(final.encode('utf-8')), filename="script.txt"))

@bot.command(name="obf")
async def obfuscate_lua(ctx, *, text_code: str = None):
    balance = get_coins(ctx.author.id)
    if ctx.author.id != FREE_USER_ID and balance < COST:
        await ctx.send(f"Insufficient funds. Need {COST}.")
        return
    lua_content = ""
    if ctx.message.attachments:
        lua_content = (await ctx.message.attachments[0].read()).decode("utf-8")
    elif text_code:
        lua_content = text_code.strip("`").replace("lua\n", "", 1)
    else:
        await ctx.send("No file/code.")
        return
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(XHIDER_URL, data=payload) as response:
            if response.status == 200:
                ...
                if ctx.author.id != FREE_USER_ID: set_coins(ctx.author.id, balance - COST)
                fixed = code.replace("--// This file was created by XHider v1.2 [https://discord.gg/hATuHQaQRb]", "-- Created by 8xmj https://discord.gg/swjkGWeDM --")
                await progress.delete()
                await ctx.send(content=f"Done, {ctx.author.mention}", file=discord.File(io.BytesIO(fixed.encode("utf-8")), filename="obf.lua"))
            else: await progress.edit(content="Error")
